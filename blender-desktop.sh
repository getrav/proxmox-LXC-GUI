...
if (whiptail --title "Blender Desktop LXC" --yesno "This will create a New Blender Desktop LXC. Proceed?" 10 58); then
...
function default_settings() {
    ...
    VLAN=""
		echo -e "${BL}Creating a Blender Desktop LXC using the above default settings${CL}"
}
...
if (whiptail --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create Blender Desktop LXC?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating a Blender Desktop LXC using the above advanced settings${CL}"
else
  clear
  header_info
...
lxc-attach -n $CTID -- bash -c "$(wget -qLO - https://raw.githubusercontent.com/getrav/proxmox-LXC-GUI/main/setup/blender-install.sh)" || exit
IP=$(pct exec $CTID ip a s dev eth0 | sed -n '/inet / s/\// /p' | awk '{print $2}')
pct set $CTID -description "# Blender Desktop LXC"
msg_ok "Completed Successfully!\n"
echo -e "Blender Desktop should be available via a VNC/RDP client or display passthrough
             \n"
